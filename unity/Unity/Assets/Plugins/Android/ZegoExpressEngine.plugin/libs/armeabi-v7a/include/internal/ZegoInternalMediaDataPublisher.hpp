...
namespace ZEGO
{
    namespace EXPRESS
    {
        ...
        {
        public:
            ZegoExpressMediaDataPublisherImp(zego_instance index)
            ...
            void reset() override
            {
                oInternalOriginBridge->mediaDataPublisherReset(instance_index_);
            }
            ...
            void setMediaDataPublisherEventHandler(std::shared_ptr<IZegoMediaDataPublisherEventHandler> handler) override
            {
                std::lock_guard<std::mutex> lock(media_data_publisher_mutex_);
                media_data_publisher_event_handler = handler;
            }

            void zego_on_media_data_publisher_file_open(const std::string& path, zego_instance instance_index)
            {
                std::lock_guard<std::mutex> lock(media_data_publisher_mutex_);
                if(media_data_publisher_event_handler)
                {
                    auto weak_mdp_event_handler = std::weak_ptr<IZegoMediaDataPublisherEventHandler>(media_data_publisher_event_handler);
            ...
            void zego_on_media_data_publisher_file_close(zego_error error_code, const std::string& path, zego_instance instance_index)
            {
                std::lock_guard<std::mutex> lock(media_data_publisher_mutex_);
                if(media_data_publisher_event_handler)
                {
            ...
            void zego_on_media_data_publisher_file_data_begin(const std::string& path, zego_instance instance_index)
            {
                std::lock_guard<std::mutex> lock(media_data_publisher_mutex_);
                if(media_data_publisher_event_handler)
                {
