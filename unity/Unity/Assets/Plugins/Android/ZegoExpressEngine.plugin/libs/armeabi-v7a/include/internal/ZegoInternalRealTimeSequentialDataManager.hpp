...
namespace ZEGO
{
    namespace EXPRESS
    {
        ...
        {
        public:
            ZegoExpressRealTimeSequentialDataManagerImp(int index)
                : instance_index_(index)
            {
            }

            void setEventHandler(std::shared_ptr<IZegoRealTimeSequentialDataEventHandler> handler) override
            {
            ...
            {
                oInternalOriginBridge->realTimeSequentialDataStartBroadcasting(streamID.c_str(), instance_index_);
            }

            void stopBroadcasting(const std::string& streamID) override
            {
                oInternalOriginBridge->realTimeSequentialDataStopBroadcasting(streamID.c_str(), instance_index_);
            }

            void sendRealTimeSequentialData(const unsigned char* data, unsigned int dataLength, const std::string& streamID, ZegoRealTimeSequentialDataSentCallback callback) override
            {
                int seq = oInternalOriginBridge->realTimeSequentialDataSendData(data, dataLength, streamID.c_str(), instance_index_);
                if (callback != nullptr)
                {
                    std::lock_guard<std::mutex> lock(event_handler_mutex_);
            ...
            {
                oInternalOriginBridge->realTimeSequentialDataStartSubscribing(streamID.c_str(), instance_index_);
            }

            void stopSubscribing(const std::string& streamID) override
            {
                oInternalOriginBridge->realTimeSequentialDataStopSubscribing(streamID.c_str(), instance_index_);
            }
            ...
            void zego_on_receive_real_time_sequential_data(const unsigned char* data, unsigned int data_length, const char * stream_id)
            {
                std::lock_guard<std::mutex> lock(event_handler_mutex_);
                if (event_handler_)
                {
                    auto weak_event_handler = std::weak_ptr<IZegoRealTimeSequentialDataEventHandler>(event_handler_);
                    ...
                    if (handler_in_main)
                    {
                        handler_in_main->onReceiveRealTimeSequentialData(this, (const unsigned char *)buffer.data(), ZegoUInt(buffer.size()), streamID);
                    }
                }
            }
