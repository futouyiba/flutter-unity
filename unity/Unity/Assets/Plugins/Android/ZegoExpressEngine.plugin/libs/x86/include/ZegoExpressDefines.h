...
namespace ZEGO {
    namespace EXPRESS {
        ...
        struct ZegoPlayerConfig
        {
            ...
            ZegoPlayerConfig(): resourceMode(ZEGO_STREAM_RESOURCE_MODE_DEFAULT), cdnConfig(nullptr), videoLayer((ZegoPlayerVideoLayer)99), videoCodecID(ZEGO_VIDEO_CODEC_ID_UNKNOWN), roomID(""), sourceResourceType(ZEGO_RESOURCE_TYPE_RTC){
            }
        ...
        struct ZegoMixerOutputVideoConfig
        {
            ...
            ZegoMixerOutputVideoConfig(): videoCodecID(ZEGO_VIDEO_CODEC_ID_DEFAULT), bitrate(0), encodeProfile(ZEGO_ENCODE_PROFILE_DEFAULT), encodeLatency(0){
            }

            ZegoMixerOutputVideoConfig(ZegoVideoCodecID codecID, int bitrate): videoCodecID(codecID), bitrate(bitrate), encodeProfile(ZEGO_ENCODE_PROFILE_DEFAULT), encodeLatency(0){
            }

            ZegoMixerOutputVideoConfig(ZegoVideoCodecID codecID, int bitrate, ZegoEncodeProfile profile, int encodeLatency): videoCodecID(codecID), bitrate(bitrate), encodeProfile(profile), encodeLatency(encodeLatency){
            }
        ...
        struct ZegoFontStyle
        {
             ...
             */
            ZegoFontStyle():type(ZEGO_FONT_TYPE_SOURCE_HAN_SANS),size(24),color(16777215),transparency(0),border(false),borderColor(0){
            }
        ...
        struct ZegoMixerInput
        {
            ...
            ZegoMixerInput() : streamID(), contentType(ZEGO_MIXER_INPUT_CONTENT_TYPE_VIDEO), layout(), soundLevelID(0), volume(100), isAudioFocus(false), audioDirection(-1), label(""), renderMode(ZEGO_MIX_RENDER_MODE_FILL){
            }

            ZegoMixerInput(std::string streamID, ZegoMixerInputContentType contentType, ZegoRect layout, unsigned int soundLevelID): streamID(streamID), contentType(contentType), layout(layout), soundLevelID(soundLevelID), volume(100),isAudioFocus(false), audioDirection(-1), label(""), renderMode(ZEGO_MIX_RENDER_MODE_FILL){
            }
        ...
        struct ZegoMixerTask
        {
            ...
            ZegoMixerTask(std::string taskID=""): taskID(taskID), watermark(nullptr), backgroundColor(0), enableSoundLevel(false), streamAlignmentMode(ZEGO_STREAM_ALIGNMENT_MODE_NONE), userData(nullptr), userDataLength(0){
            }
        ...
        struct ZegoAutoMixerTask
        {
             ...
             */
            ZegoAutoMixerTask(): taskID(), roomID(), outputList(),audioConfig(),enableSoundLevel(false),streamAlignmentMode(ZEGO_STREAM_ALIGNMENT_MODE_NONE){
            }
         ...
         */
        using ZegoCopyrightedMusicSendExtendedRequestCallback = std::function<void (int errorCode, std::string command, std::string result)>;
         ...
         */
        using ZegoCopyrightedMusicGetLrcLyricCallback = std::function<void (int errorCode, std::string lyrics)>;
         ...
         */
        using ZegoCopyrightedMusicGetKrcLyricByTokenCallback = std::function<void (int errorCode, std::string lyrics)>;
         ...
         */
        using ZegoCopyrightedMusicRequestSongCallback = std::function<void (int errorCode, std::string resource)>;
         ...
         */
        using ZegoCopyrightedMusicRequestAccompanimentCallback = std::function<void (int errorCode, std::string resource)>;
         ...
         */
        using ZegoCopyrightedMusicRequestAccompanimentClipCallback = std::function<void (int errorCode, std::string resource)>;
         ...
         */
        using ZegoCopyrightedMusicGetMusicByTokenCallback = std::function<void (int errorCode, std::string resource)>;
         ...
         */
        using ZegoCopyrightedMusicGetStandardPitchCallback = std::function<void (int errorCode, std::string pitch)>;
