...
namespace ZEGO
{
    namespace EXPRESS
    {
        ...
        public:
            ...
            void setDummyCaptureImagePath(const std::string& filePath, ZegoPublishChannel channel = ZEGO_PUBLISH_CHANNEL_MAIN) override
            {
            ...
            void loginRoom(const std::string &roomID, ZegoUser user, ZegoRoomConfig config, ZegoRoomLoginCallback callback) override
            {
                const char *_roomId = roomID.c_str();
                zego_user _user = ZegoExpressConvert::O2IUser(user);
            ...
            void startPublishingStream(const std::string &streamID, ZegoPublisherConfig config, ZegoPublishChannel channel) override
            {
                const char *stream_id = streamID.c_str();
                zego_publisher_config _config = ZegoExpressConvert::O2IPublisherConfig(config);
                oInternalOriginBridge->startPublishingStreamWithConfig(stream_id, zego_publish_channel(channel), _config);
            }
            ...
            void enablePlayStreamVirtualStereo(bool enable, int angle, const std::string& streamID) override
            {
            ...
            {
                oInternalOriginBridge->setElectronicEffects(enable, zego_electronic_effects_mode(mode), tonal);
            }
            ...
            void setMinVideoResolutionForTrafficControl(int width, int height, ZegoPublishChannel channel) override
            {
                oInternalOriginBridge->setMinVideoResolutionForTrafficControl(width, height, zego_publish_channel(channel));
            }
#endif
            ...
            void sendSEISyncWithCustomVideo(const unsigned char* data, unsigned int dataLength, unsigned long long timeStampNs, ZegoPublishChannel channel)
            {
                oInternalOriginBridge->sendSEISyncWithCustomVideo(data, dataLength, timeStampNs, (zego_publish_channel)channel);
            }

            void sendAudioSideInfo(const unsigned char* data, unsigned int dataLength, double timeStampMs, ZegoPublishChannel channel)
            {
                oInternalOriginBridge->sendAudioSideInfo(data, dataLength, timeStampMs, (zego_publish_channel)channel);
            }
            ...
            {
                return oInternalOriginBridge->isVideoEncoderSupported(static_cast<zego_video_codec_id>(codecID));
            }
            ...
            {
                return oInternalOriginBridge->isVideoDecoderSupported(static_cast<zego_video_codec_id>(codecID));
            }
            ...
            {
                oInternalOriginBridge->setCameraFocusMode(zego_camera_focus_mode(mode), zego_publish_channel(channel));
            }
            ...
            {
                oInternalOriginBridge->setCameraExposureMode(zego_camera_exposure_mode(mode), zego_publish_channel(channel));
            }
            ...
            {
                zego_device_info device_info_src = oInternalOriginBridge->getCurrentAudioDevice(zego_audio_device_type(deviceType));
...
#if ZEGO_EXPRESS_VIDEO_SDK
            void startEffectsEnv() override
            {
                oInternalOriginBridge->startEffectsEnv();
            }

            void stopEffectsEnv() override
            {
                oInternalOriginBridge->stopEffectsEnv();
            }
            ...
            IZegoRealTimeSequentialDataManager* createRealTimeSequentialDataManager(const std::string& roomID) override {
            ...
            void destroyRealTimeSequentialDataManager(IZegoRealTimeSequentialDataManager*& manager) override {
                if (manager && oInternalCallbackCenter->getZegoExpressRealTimeSequentialDataManagerImp(manager->getIndex())) {
                    oInternalOriginBridge->destroyRealTimeSequentialDataManager(manager->getIndex());
                    oInternalCallbackCenter->eraseZegoExpressRealTimeSequentialDataManagerImp(manager->getIndex());
                }
            }
            ...
            void StartOrStopMixerTask(bool isStart, ZegoMixerTask task, ZegoMixerStartCallback startCallback, ZegoMixerStopCallback stopCallback)
            {
                ...
                std::vector<std::shared_ptr<zego_mixer_output_video_config>> output_video_config_list;
                {
                    ...
                    {
                        std::shared_ptr<zego_mixer_output_video_config> videoConfig = std::make_shared<zego_mixer_output_video_config>();
                        ...
                        videoConfig->video_codec_id = static_cast<zego_video_codec_id>(output.videoConfig.videoCodecID);
                        videoConfig->encode_latency = output.videoConfig.encodeLatency;
                        videoConfig->encode_profile = static_cast<zego_encode_profile>(output.videoConfig.encodeProfile);
            ...
            void StartOrStopAutoMixerTask(bool isStart, ZegoAutoMixerTask task, ZegoMixerStartCallback startCallback, ZegoMixerStopCallback stopCallback)
            {
            ...
            void destroyRangeAudio(IZegoRangeAudio *&rangeAudio) override
            {
                if (rangeAudio /*&& oInternalCallbackCenter->getZegoExpressRangeAudioImp(rangeAudio->getIndex()*/)
                {
                    ...
                    oInternalCallbackCenter->eraseZegoExpressRangeAudioImp(zego_range_audio_instance_index_first);
                }
            }
            ...
            {
                std::lock_guard<std::recursive_mutex> locker(oCopyrightedMusicContainer->copyrightedMusicMutex);
                if (copyrightedMusic != nullptr && copyrightedMusic == oCopyrightedMusicContainer->copyrightedMusicInstance.get())
                {
            ...
            IZegoMediaDataPublisher* createMediaDataPublisher() override
            {
                ...
                auto media_data_publisher_instance = std::make_shared<ZegoExpressMediaDataPublisherImp>(index);
                oInternalCallbackCenter->insertZegoExpressMediaDataPublisherImp(index, media_data_publisher_instance);
...
#if ZEGO_EXPRESS_VIDEO_SDK
            void setCustomVideoCaptureDeviceState(bool isEnable, ZegoRemoteDeviceState state, ZegoPublishChannel channel = ZEGO_PUBLISH_CHANNEL_MAIN) override
            {
                oInternalOriginBridge->setCustomVideoCaptureDeviceState(isEnable, zego_remote_device_state(state), zego_publish_channel(channel));
            }
#endif

#if ZEGO_EXPRESS_VIDEO_SDK
            void setCustomVideoCaptureRegionOfInterest(std::vector<ZegoRoiRect> rects, ZegoPublishChannel channel = ZEGO_PUBLISH_CHANNEL_MAIN) override
            {
                ...
                oInternalOriginBridge->customVideoCaptureRegionOfInterest(c_rects.data(), c_rects.size(), zego_publish_channel(channel));
            }
#endif
            ...
            void enableCustomAudioCaptureProcessingAfterHeadphoneMonitor(bool enable, ZegoCustomAudioProcessConfig *config) override
            {
                if (config)
                {
                    auto _config = ZegoExpressConvert::O2ICustomAudioProcessConfig(*config);
                    oInternalOriginBridge->enableCustomAudioCaptureProcessingAfterHeadphoneMonitor(enable, &_config);
                }
                else
                {
                    oInternalOriginBridge->enableCustomAudioCaptureProcessingAfterHeadphoneMonitor(enable, nullptr);
                }
            }
            ...
            void enableCustomAudioPlaybackProcessing(bool enable, ZegoCustomAudioProcessConfig* config) override
            {
                if (config)
                {
                    auto _config = ZegoExpressConvert::O2ICustomAudioProcessConfig(*config);
        ...
        public:
            ...
            static void setEngineConfig(ZegoEngineConfig engineConfig)
            {
                ...
                {
                    strncpy(_logConfig.log_path, engineConfig.logConfig->logPath.c_str(), ZEGO_EXPRESS_MAX_COMMON_LEN);
...
#define oEngineContainer ZegoSingleton<ZegoExpressEngineContainer>::CreateInstance()
        ...
        public:
            ...
            static IZegoExpressEngine *createEngine(ZegoEngineProfile profile, std::shared_ptr<IZegoEventHandler> eventHandler)
            {
                std::lock_guard<std::recursive_mutex> locker(oEngineContainer->engineMutex);
