...
namespace ZEGO
{
    namespace EXPRESS
    {
        ...
        public:
            ...
            void loadResourceWithPosition(const std::string& sourcePath, unsigned long long startPosition, ZegoMediaPlayerLoadResourceCallback callback)
            {
                int seq = oInternalOriginBridge->mediaPlayerPreloadWithPosition(sourcePath.c_str(), startPosition, zego_media_player_instance_index(instanceIndex));
                ZEGO_UNUSED_VARIABLE(seq);
                if (callback != nullptr)
                {
                    std::lock_guard<std::mutex> lock(mediaEventMutex);
            ...
            void loadResourceFromMediaData(unsigned char* media_data, int media_data_length, unsigned long long start_position, ZegoMediaPlayerLoadResourceCallback callback) override
            {
                int seq = oInternalOriginBridge->mediaPlayerPreload(media_data, media_data_length, start_position, zego_media_player_instance_index(instanceIndex));
                ZEGO_UNUSED_VARIABLE(seq);
                if (callback != nullptr)
                {
                    std::lock_guard<std::mutex> lock(mediaEventMutex);
            ...
            void loadCopyrightedMusicResourceWithPosition(const std::string& resourceId, unsigned long long startPosition, ZegoMediaPlayerLoadResourceCallback callback)
            {
                int seq = oInternalOriginBridge->mediaPlayerPreloadCopyrightedMusicResourceWithPosition(resourceId.c_str(), startPosition, zego_media_player_instance_index(instanceIndex));
                ZEGO_UNUSED_VARIABLE(seq);
                if (callback != nullptr)
                {
                    std::lock_guard<std::mutex> lock(mediaEventMutex);
            ...
            {
                oInternalOriginBridge->mediaPlayerSetPlayLoopCount(count, zego_media_player_instance_index(instanceIndex));
            }
            
            void setPlaySpeed(float speed) override
            {
                oInternalOriginBridge->mediaPlayerSetPlaySpeed(speed, zego_media_player_instance_index(instanceIndex));
            }
            ...
            {
                oInternalOriginBridge->mediaPlayerEnableSoundLevelMonitor(enable, millisecond, zego_media_player_instance_index(instanceIndex));
            }

            void enableFrequencySpectrumMonitor(bool enable, unsigned int millisecond) override
            {
                oInternalOriginBridge->mediaPlayerEnableFrequencySpectrumMonitor(enable, millisecond, zego_media_player_instance_index(instanceIndex));
            }
            ...
            void zego_on_media_player_frequency_spectrum_update(float* spectrum_list, unsigned int spectrum_count)
            {
...
#if ZEGO_EXPRESS_VIDEO_SDK
            void zego_on_media_player_video_frame(const unsigned char **data, unsigned int *data_length, const struct zego_video_frame_param _param, const char* extra_info)
            {
                std::lock_guard<std::mutex> lock(mediaEventMutex);
                if (mpVideoHandler)
                {
